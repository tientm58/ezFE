ezCloud.factory("BEFactory", ['$http', 'loginFactory', '$rootScope', '$filter', '$mdDialog', 'dialogService', 'reservationFactory', '$timeout', '$mdMedia', 'configFactory', 'smartCardFactory', 'roomListFactory', '$q','$state','$location', function($http, loginFactory, $rootScope, $filter, $mdDialog, dialogService, reservationFactory, $timeout, $mdMedia, configFactory, smartCardFactory, roomListFactory, $q,$state,$location) {
    var CMApiUrlRoot = "http://localhost:3003/";
    var allCMData = null;
    return {
        getCMConfiguration: function() {
            var getCMConfiguration = loginFactory.securedGet("api/BookingEngine/GetBEManagerData", "");
            return getCMConfiguration;
        },
        get_CMRoomTypeMappings_CMRoomTypes: function(callback) {

            var testPromise = loginFactory.securedGet("api/BookingEngine/AllChannelManagerRoomTypeMapping");
            $rootScope.dataLoadingPromise = testPromise;
            testPromise.success(function(data){
                if(data=="BE_HOTEL_IS_NOT_REGISTER"){
                    DialogSuggest();
                    return;
                }
                ...
                if (data.CMRoomTypeMappings != null && data.CMRoomTypeMappings.length > 0) {
                    ...
                    console.log("BEFactory Data", result);
        ...
        getCMRoomTypeMappings: function(callback) {
            var getCMRoomTypeMappings = loginFactory.securedGet("api/BookingEngine/AllChannelManagerRoomTypeMapping", "");
        ...
        getAvailabilityMatrixDataCM: function(from, to, callback) {
            var f = from.format("yyyy-mm-dd");
            var t = to.format("yyyy-mm-dd");
            var getAvailabilityMatrixDataCM = loginFactory.securedGet("api/ChannelManager/GetAvailabilityMatrixData", "from=" + f + "&&to=" + t);
            $rootScope.dataLoadingPromise=getAvailabilityMatrixDataCM;
            getAvailabilityMatrixDataCM.success(function(data) {
                var avMatrixData = {};
                var avMatrixDataTemp = angular.copy(data);
                avMatrixData.CMConfiguration = avMatrixDataTemp.CMConfiguration;
                // Build Room Types Data for avMatrixData
        ...
        processCMSearchReservation:function(search,callback){
            var processCMSearchReservation = loginFactory.securedPostJSON("api/BookingEngine/SearchCMReservationProcess", search);
        ...
        removeCMRoomTypeMapping:function(RoomTypeId){
            removeCMRoomTypeMapping(RoomTypeId);
        },
        removeCMRoomRateMapping:function(CMRoomRateMappingId){
            removeCMRoomRateMapping(CMRoomRateMappingId);
        },
        CheckHotelCMConfiguration:function(){
            CheckHotelCMConfiguration();
        },
        checkIfBEModuleActive: function () {
            var ezHotelModulesList = $rootScope.HotelSettings.EzHotelModulesList;
            if (ezHotelModulesList != null && ezHotelModulesList.length > 0) {
                var BEModule = _.filter(ezHotelModulesList, function (module) {
                    return module.EzModules != null && module.EzModules.ModuleCode == "BE" && module.EzModules.Status == true && module.Status == true;
                });
                console.info("BEModule", BEModule);
                if (BEModule == null || BEModule.length == 0) {
                    var translated_title = $filter("translate")("LOG_MESSAGES");
                    var translated_content = $filter("translate")("BE_SUGGEST_MESSAGE");
    ...
    function CMRegister(register) {
        if (register != null && register.CMUser != null && register.CMPassword != null && register.CMPropertyId != null) {
            if ($rootScope.HotelSettings != null) {
                register.HotelId = $rootScope.HotelSettings.HotelId;
            }
            var data={
                CMConfigurationId:null,
                CMUser:register.CMUser,
                CMPassword:register.CMPassword,
                CMPropertyId:register.CMPropertyId,
                CMServiceUrl:null,
                IsLastRoomAvailable:null,
                ...
                SourceId:null,
                IsAllowOverBooking:null,
                IsAutoAssignRoom:null,
                TypeCms : "BOOKINGENGINE"
            };
            console.log('CM Register',data);
            var CMConfigurationRegister = loginFactory.securedPostJSON("api/BookingEngine/SaveCMConfiguration", data);
            $rootScope.dataLoadingPromise = CMConfigurationRegister;    
            CMConfigurationRegister.success(function(data) {
    ...
    function CMDeregister(CMConfiguration){
       if (CMConfiguration != null && CMConfiguration.CMUser != null && CMConfiguration.CMPassword != null && CMConfiguration.CMPropertyId != null && CMConfiguration.CMServiceUrl != null) {
            var data={
                CMConfigurationId:null,
                CMUser:CMConfiguration.CMUser,
                CMPassword:CMConfiguration.CMPassword,
                CMPropertyId:CMConfiguration.CMPropertyId,
                CMServiceUrl:null,
                IsLastRoomAvailable:false,
                HotelId:CMConfiguration.HotelId,
                CreatedDate:null,
                CreatedUserId:null,
                LastModifiedDate:null,
                LastModifiedUserId:null,
                ...
                IsAutoAssignRoom:false,
                TypeCms : "DELETE"
            };
            var BEDeregister = loginFactory.securedPostJSON("api/BookingEngine/SaveCMConfiguration", data);
            $rootScope.dataLoadingPromise = BEDeregister;
            BEDeregister.success(function(data) {
    ...
    function getCMRoomTypes() {
        var hotelId = $rootScope.HotelSettings.HotelId;
        var token = $rootScope.HotelSettings.CMServiceUrl;
        var data = {
            hotelId: hotelId,
            token:token
        };
    ...
    function removeCMRoomTypeMapping(CMRoomTypeMappingId){
        var removeCMRoomTypeMapping = loginFactory.securedPost("api/BookingEngine/RemoveCMRoomTypeMapping?CMRoomTypeMappingId="+CMRoomTypeMappingId);
        $rootScope.dataLoadingPromise = removeCMRoomTypeMapping;
        removeCMRoomTypeMapping.success(function(data) {
            dialogService.toast("REMOVE_CHANNEL_MANAGER_MAPPING_SUCCESSFULL");
            $state.go($state.current, {}, {
                reload: true
            });
        }).error(function(err) {
    ...
    function removeCMRoomRateMapping(CMRoomRateMappingId){
        var removeCMRoomRateMapping = loginFactory.securedPost("api/BookingEngine/RemoveCMRoomRateMapping?CMRoomRateMappingId="+CMRoomRateMappingId);
        $rootScope.dataLoadingPromise = removeCMRoomRateMapping;
        removeCMRoomRateMapping.success(function(data) {
            dialogService.toast("REMOVE_ROOM_RATE_CHANNEL_MANAGER_MAPPING_SUCCESSFULL");
            $state.go($state.current, {}, {
                reload: true
            });
        }).error(function(err) {
    ...
    function UpdateCMRegister(CMConfiguration){
        var UpdateCMRegister = loginFactory.securedPostJSON("api/BookingEngine/SaveCMConfiguration",CMConfiguration)
        ...
        var CheckCountCMConfigIsActive = loginFactory.securedPostJSON("api/BookingEngine/CheckCMSConfigIsActive","")
    ...
    function UpdateNumberPrioritize(CMConfiguration){
        var UpdateCMRegister = loginFactory.securedPostJSON("api/BookingEngine/UpdateNumberPrioritize",CMConfiguration)
        $rootScope.dataLoadingPromise = UpdateCMRegister;
        UpdateCMRegister.success(function (data) {
            dialogService.toast("UPDATE_SUCCESSFUL");
            $state.go($state.current, {}, {
    ...
    function CheckHotelCMConfiguration(){
        var HotelCMConfiguration = loginFactory.securedGet("api/BookingEngine/GetCMConfigurationBE","")
