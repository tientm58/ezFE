ezCloud.controller('BEAvailabilityMatrixController', ['$scope', '$rootScope', '$state', 'dialogService', '$localStorage', '$http', '$sessionStorage', 'loginFactory', '$mdDialog', 'flowFactory', '$filter', '$mdSidenav', '$mdMedia', 'configFactory', '$q', '$location', '$log', 'roomListFactory', 'frontOfficeFactory', '$timeout', 'BEFactory',
    function ($scope, $rootScope, $state, dialogService, $localStorage, $http, $sessionStorage, loginFactory, $mdDialog, flowFactory, $filter, $mdSidenav, $mdMedia, configFactory, $q, $location, $log, roomListFactory, frontOfficeFactory, $timeout, BEFactory) {
        ...
        function InitAvMatrix() {
            ...
            BEFactory.getAvailabilityMatrixData($scope.matrixFilter.startDate, $scope.matrixFilter.startDate.addDays($scope.matrixFilter.daysToShow - 1), function (data) {
                BEFactory.CheckHotelCMConfiguration();
                BEFactory.getAllRoomTypeIsMapped(function (allRoomTypeIsMapped){
                    $scope.allRoomTypeIsMapped = allRoomTypeIsMapped.allCountRoomTypeIdIsMapped;
                    $scope.CMSIsActived = allRoomTypeIsMapped.CMSIsActived;
                    $scope.BEIsActived = allRoomTypeIsMapped.BEIsActived;
                
                BEFactory.getAvailabilityMatrixDataCM($scope.matrixFilter.startDate, $scope.matrixFilter.startDate.addDays($scope.matrixFilter.daysToShow - 1), function (dataCM) {
                    $scope.avMatrixDataCM = dataCM;
                    console.log("avMatrixDataCM", $scope.avMatrixDataCM);
                    for(var i in data.days){
                        for(var j in $scope.avMatrixDataCM.days){
                            if(data.days[i].dayOfMonth == $scope.avMatrixDataCM.days[j].dayOfMonth){
                                for(var x in data.days[i].RoomTypes){
                                    if(!CheckRoomTypeIdIsMappedCMS(data.days[i].RoomTypes[x].RoomTypeId)||$scope.CMSIsActived == false){
                                        continue;
                                    }
                                    for(var y in $scope.avMatrixDataCM.days[j].RoomTypes){                                     
                                        if(data.days[i].RoomTypes[x].RoomTypeId == $scope.avMatrixDataCM.days[j].RoomTypes[y].RoomTypeId){
                                            data.days[i].RoomTypes[x].CanBeSoldCM = $scope.avMatrixDataCM.days[j].RoomTypes[y].CanBeSold;
                                        }                          
                                    }
                                    if(data.days[i].RoomTypes[x].CanBeSoldCM == undefined || data.days[i].RoomTypes[x].CanBeSoldCM == null){
                                        data.days[i].RoomTypes[x].CanBeSoldCM = 'N/A';
                                    }
                                }
                            }
                        }
        ...
        $scope.processSearch = function () {
            ...
            var GetInventoryLogData = loginFactory.securedPostJSON("api/BookingEngine/GetInventoryLogData", $scope.search);
        ...
        function InitInventoryLog() {
            var getRoomType = loginFactory.securedGet("api/BookingEngine/GetRoomType", "");
        ...
        $scope.updateAvMatrix = function (startDate, daysToShow) {
            ...
            BEFactory.getAvailabilityMatrixData(startDayTemp, startDayTemp.addDays(daysToShow - 1), function (data) {      
                BEFactory.CheckHotelCMConfiguration();
                BEFactory.getAllRoomTypeIsMapped(function (allRoomTypeIsMapped){
                    $scope.allRoomTypeIsMapped = allRoomTypeIsMapped.allCountRoomTypeIdIsMapped;
                    $scope.CMSIsActived = allRoomTypeIsMapped.CMSIsActived;
        ...
        $scope.UpdateStopSellLimit = function() {
            ...
            var updateOTA = loginFactory.securedPostJSON("api/BookingEngine/UpdateStopSellLimit", listCMRoomTypeMappingId);
        ...
        $scope.saveAvailabilityMatrixData = function (roomType, day, action) {
            if (action == "CAN_BE_SOLD") {
                ...
                var saveAvailabilityMatrixData = loginFactory.securedPostJSON("api/BookingEngine/ProcessSaveAvailabilityMatrixData", CMAvailabilityMatrixModel);
        ...
        $scope.avBulkUpdate = function () {
            $mdDialog.show({
                ...
                if (bulkUpdateModel != null) {
                    var processSaveBulkUpdate = loginFactory.securedPostJSON("api/BookingEngine/ProcessSaveBulkUpdate", bulkUpdateModel);
            ...
            function CMAVBulkUpdateDialogController($scope, $mdDialog, loginFactory, dialogService, roomTypes, avMatrixData) {
                function Init() {
                    ...
                    else if (avMatrixData.CMConfiguration.TypeCms == "BOOKINGENGINE") {
                        $scope.updateTypes = [{
                            value: 1,
                            label: "AVAILABILITY"
                        }, {
        ...
        function CheckRoomTypeIdIsMappedCMS(roomTypeId) {
        ...
        $scope.checkRoomTypeIdIsMappedCMS = function (roomTypeId) {
            if($scope.allRoomTypeIsMapped == undefined || $scope.allRoomTypeIsMapped==null){
                return false;
            }
            for(var i in $scope.allRoomTypeIsMapped){
