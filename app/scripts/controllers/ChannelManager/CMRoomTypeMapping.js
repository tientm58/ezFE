ezCloud.controller('CMRoomTypeMappingController', ['$scope', '$rootScope', '$state', 'dialogService', '$localStorage', '$http', '$sessionStorage', 'loginFactory', '$mdDialog', 'flowFactory', '$filter', '$mdSidenav', '$mdMedia', 'configFactory', '$q', '$location', '$log', 'roomListFactory', 'frontOfficeFactory', '$timeout', 'CMFactory',
    function ($scope, $rootScope, $state, dialogService, $localStorage, $http, $sessionStorage, loginFactory, $mdDialog, flowFactory, $filter, $mdSidenav, $mdMedia, configFactory, $q, $location, $log, roomListFactory, frontOfficeFactory, $timeout, CMFactory) {
        function Init() {
            ...
            CMFactory.get_CMRoomTypeMappings_CMRoomTypes(function (data) {
                ...
                for (var i = 0; i < $scope.mappedRoomTypes.length; i++) {
                    if ($scope.mappedRoomTypes[i].RoomPrice_Rate_MappedList != null && $scope.mappedRoomTypes[i].RoomPrice_Rate_MappedList != undefined) {
                        var temp = $scope.mappedRoomTypes[i].RoomPrice_Rate_MappedList;
                        var isOk = true;
                        for (var j = 0; j < temp.length; j++) {
                            if (temp[j].CMRoomTypeMapping == null || temp[j].CMRoomTypeMapping == undefined || temp[j].CMRoomTypeMapping.length == 0) {
                                isOk = false;
                            }
                            if (j == temp.length - 1 && !$scope.mappedRoomTypes[i].isRanking) {
                                if (!isOk) $scope.mappedRoomTypes[i].isRanking = false;
                                else $scope.mappedRoomTypes[i].isRanking = true;
                            }
                        }
                    }
                }
                $scope.isKeepRanking = $scope.mappedRoomTypes[$scope.mappedRoomTypes.length - 3] == 1 ? true : false;
                $scope.isShowKeepRanking = $scope.mappedRoomTypes[$scope.mappedRoomTypes.length - 3] == 0 ? false : true;
                $scope.isEditRanking = false;
            });
            $scope.typeCms = $rootScope.dataCMS;
        }
        Init();
        $scope.mapMoreCMRoomType = function (roomType) {
            ...
            $mdDialog.show({
                ...
                if (availableRoomTypeMapping != null && availableRoomTypeMapping.length > 0) {
                    var AddCMRoomTypeMapping = loginFactory.securedPostJSON("api/ChannelManager/AddCMRoomTypeMapping", availableRoomTypeMapping[0]);
        ...
        $scope.mapMoreCMRoomRate = function (roomPrice) {
            ...
            $mdDialog.show({
                ...
                if (availableRoomRateMapping != null && availableRoomRateMapping.length > 0) {
                    var AddCMRoomRateMapping = loginFactory.securedPostJSON("api/ChannelManager/AddCMRoomRateMapping", availableRoomRateMapping[0]);
        ...
        $scope.removeCMRoomType = function (roomType) {
            dialogService.confirm("WARNING", "DO_YOU_WANT_TO_REMOVE_ROOM_TYPE_AND_ROOM_RATE_CHANNEL_MANAGER" + "?").then(function () {
                ...
                CMFactory.removeCMRoomTypeMapping(RoomTypeId, function(data){
        ...
        $scope.removeCMRoomRate = function (roomPrice) {
            dialogService.confirm("WARNING", "DO_YOU_WANT_TO_REMOVE_ROOM_RATE_CHANNEL_MANAGER" + "?").then(function () {
                ...
                CMFactory.removeCMRoomRateMapping(CMRoomRateMappingId, function(data){
        ...
        $scope.isCheckKeepRanking = function () {
            $scope.isEditRanking = true;
            $scope.isKeepRanking = !$scope.isKeepRanking;
        };
        $scope.changeKeepRanking = function () {
            $scope.isEditRanking = true;
        };
        $scope.saveKeepRanking = function () {
            var cmRoomTypesMappingsTemp = [];
            for (var i = 0; i < $scope.mappedRoomTypes.length; i++) {
                if ($scope.mappedRoomTypes[i].MappedTo != null && $scope.mappedRoomTypes[i].MappedTo != undefined) {
                    if ($scope.mappedRoomTypes[i].MappedTo.length > 0) {
                        cmRoomTypesMappingsTemp.push($scope.mappedRoomTypes[i].MappedTo[0]);
                    }
                }
            }
            var data = {
                isKeppRanking: $scope.isKeepRanking,
                cmRoomTypeMappings: cmRoomTypesMappingsTemp
            };
            var SaveRanking = loginFactory.securedPostJSON("api/ChannelManager/SaveKeepRanking", data);
            $rootScope.dataLoadingPromise = SaveRanking;
            SaveRanking.success(function (data) {
                dialogService.toast("KEEP_RANKING_SUCCESSFULL");
                $scope.isEditRanking = false;
            }).error(function (err) {
                console.log(err);
            });
