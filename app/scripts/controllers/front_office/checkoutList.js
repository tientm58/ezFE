ezCloud.controller('checkoutListController', ['$scope', '$rootScope', '$state', 'SharedFeaturesFactory', 'dialogService', '$localStorage', '$http', '$sessionStorage', 'loginFactory', '$mdDialog', 'flowFactory', '$filter', '$mdSidenav', '$mdMedia', 'configFactory', '$q', '$location', '$log', 'roomListFactory', 'frontOfficeFactory', '$timeout','$window',
function ($scope, $rootScope, $state, SharedFeaturesFactory, dialogService, $localStorage, $http, $sessionStorage, loginFactory, $mdDialog, flowFactory, $filter, $mdSidenav, $mdMedia, configFactory, $q, $location, $log, roomListFactory, frontOfficeFactory, $timeout,$window) {
		...
		var Search = function () {
			...
			/*if ($scope.search.From > $scope.search.To) {
                ...
                return;
					...
					if (isNaN(Date.parse($scope.fromString)) || isNaN(Date.parse($scope.toString)) ){
									dialogService.messageBox("INVALID_FROM/TO_DATE", "PLEASE_SELECT_A_VALID_DATE_TO_PERFORM_SEARCH_ACTION");
									return;
						}*/
		...
		$scope.menuItemClick = function (item, result) {
			...
			if (item.name === 'PRE_CHECKOUT') {
				var el = document.getElementById('search');
				frontOfficeFactory.processChangeReservationStatus(item, result, el, "PRE_CHECKOUT");
			}
		...
		function ShowCheckoutDetailDialogController($scope, $mdDialog, checkoutDetail) {
			...
			$scope.checkoutDetail = checkoutDetail;
			console.log("ROOM DETAIL", $scope.checkoutDetail);
		}
		...
		$scope.showDepartureDetail = function (result, event) {
			...
			$mdDialog.show({
				controller: ShowCheckoutDetailDialogController,
				resolve: {
					checkoutDetail: function () {
						return resultTemp;
					}
				},
				templateUrl: 'views/templates/showCheckoutDetail.tmpl.html',
